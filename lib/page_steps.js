...
const { setFileDownloadPath } = require("./_init");
...
step(["Goto <url>", "Open <url>", "Goto <url> in new tab", "Open <url> in new tab"], async url => {
  ...
  const stepName = helpers.getScenarioData("step name");
  let isNewTab = stepName.endsWith("in new tab");
  if (isNewTab) {
...
step(["Close window", "Close window tab"], async () => {
...
step(["Clear and enter <value> to <element> text box", "Clear and enter <value> to <element>"], async (value, element) => {
  value = helpers.getValue(value);
  gauge.message("Entering " + value);
...
step(["Save <element> as <elementTextKey>", "Save <element> text as <elementTextKey>"], async (element, elementTextKey) => {
  ...
  let elementText = tagName === "INPUT" || tagName === "TEXTAREA" || tagName === "SELECT" ?
