...
step(["Open <url>", "Goto <url>"], async url => {
  const navigateUrl = process.env[url] ? process.env[url] : (url ? url : process.env.APP_ROOT_URL);
...
step("Scroll to <element>", async element => {
  taiko.scrollTo(await getPageElement(taiko, element), { blockAlignment: 'center', inlineAlignment: 'center' })
});
...
step(["Click <element> <elementText>", "Click on <element> <elementText>"], async (element, elementText) => {
  element = element + " with text " + helpers.getScenarioData(elementText);
  await taiko.click(await getPageElement(taiko, element));
});

step(["Clear <element> text box", "Clear <element>"], async element => {
  const pageElement = element.startsWith("textBox") ?
    taiko.textBox(element.replace("textBox", "").trim()) : await getPageElement(taiko, element);
...
step(["Save <element> as <elementTextKey>", "Save <element> text as <elementTextKey>"], async (element, elementTextKey) => {
  const elementText = (await (await getPageElement(taiko, element)).text()).toString();
