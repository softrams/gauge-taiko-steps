...
beforeSuite(async () => {
  if (!browser_for_each_scenario || browser_for_each_scenario === 'false') {
    ...
    taiko.setConfig({
      ...
      // waitForEvents: Number((process.env.wait_for_events || taiko.getConfig("waitForEvents"))),
      highlightOnAction: (process.env.highlight_on_action || taiko.getConfig("highlightOnAction"))
    });
  }
  ...
  console.log("highlightOnAction => - " + taiko.getConfig("highlightOnAction"));
});
